...
func newDBConnection() (*pgxpool.Pool, error) {
	...
	connString := makeDNS(user, pass, host, port, dbName, sslMode, min, max)
	config, err := pgxpool.ParseConfig(connString)
	if err != nil {
		return nil, fmt.Errorf("%s %w", "pgxpool.ParseConfig(connString)", err)
	}

	config.ConnConfig.RuntimeParams["aplication_name"] = AppName

	pool, err := pgxpool.NewWithConfig(context.Background(), config)
	if err != nil {
		return nil, fmt.Errorf("%s %w", "pgxpool.NewWithConfig(context.Background(), config)", err)
	}
